...
import { getAllOrders, getOrderById, createOrder, updateOrder, deleteOrder, approveOrder, shipOrder } from "@/services/orderService";
...
export function useOrderMutation({ type, id, onSuccess }: UseOrderMutationOptions) {
  ...
  return useMutation({
    mutationKey: [type === "update" || type === "delete" ? `${type}Order` : "createOrder", id],
    mutationFn,
    onSuccess: () => {
