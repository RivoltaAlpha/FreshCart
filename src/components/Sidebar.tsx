...
export function Sidebar({ userType, currentPage, onPageChange, sidebarOpen, sidebarToggle, onClose }: {
  ...
  return (
    <>
      ...
      {!sidebarOpen && (
        <button
          className="fixed top-4 left-4 z-50 md:hidden p-2 text-gray-600 bg-white rounded-full shadow-lg hover:text-[#05445E]"
      ...
      <div className={`md:hidden ${sidebarOpen ? 'block' : 'hidden'}`}>
        <aside className={`fixed inset-y-0 left-0 z-50 bg-[#013A66] text-white shadow-lg transform ${sidebarOpen ? 'translate-x-0 w-64' : '-translate-x-full w-16'} transition-transform duration-200 ease-in-out`}>
      ...
      <aside className={`hidden md:block inset-y-0 left-0 z-50 bg-[#013A66] text-white shadow-lg transform ${sidebarToggle ? 'translate-x-0 w-64' : '-translate-x-full w-16'} transition-transform duration-200 ease-in-out lg:translate-x-0 lg:static lg:inset-0`}>
        <div className="flex items-center justify-between p-6 border-b border-gray-200">
          <h1 className={`font-bold text-xl text-fresh-primary ${!sidebarToggle && 'hidden'}`}>
            FreshCart
          </h1>
          <button
            className="p-1 hover:bg-gray-100"
          ...
          {menus[userType]?.map((item) => {
            ...
            return (
              <button
                ...
                className={`w-full flex items-center px-6 py-4 text-left hover:bg-[#41729F] ${currentPage === item.id ? ' text-white border-l-4 border-[#189AB4]' : ''}`}
              >
                <Icon className={`w-5 h-5 ${sidebarToggle ? 'mr-3' : ''}`} />
