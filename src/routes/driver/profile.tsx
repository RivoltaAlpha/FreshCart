      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="lg:col-span-1">
            <div className="bg-white rounded-2xl shadow-lg p-6 text-center">
              ...
              <div className="inline-flex items-center gap-1 px-3 py-1 bg-[#00A7B3]/10 text-[#00A7B3] rounded-full text-sm font-medium mb-4">
                <Shield className="h-4 w-4" />
                {user?.role || authUser?.role ? (user?.role || authUser?.role).charAt(0).toUpperCase() + (user?.role || authUser?.role).slice(1) : 'driver'}
              </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div className="flex items-center gap-2 p-3 bg-gray-50 rounded-lg">
                      ...
                      <div>
                        ...
                        <p className="font-medium">{user?.role ? user.role.charAt(0).toUpperCase() + user.role.slice(1) : "driver"}</p>
                      </div>
                    </div>
                    <div className="flex items-center gap-2 p-3 bg-gray-50 rounded-lg">
        ...
        <div className="mb-8">
          <div className="bg-white rounded-2xl shadow-lg p-6 flex flex-col md:flex-row items-center justify-between">
            <div>
              <h3 className="text-lg font-semibold text-gray-800 mb-2">Driver Status</h3>
              <p className="text-gray-600 mb-2">Set your current status so stores and customers know if you are available for deliveries.</p>
            </div>
            <div className="flex gap-2">
              <button
                className={`px-4 py-2 rounded-lg font-semibold transition-colors border border-gray-200 ${authUser?.driver_status === 'active' ? 'bg-[#00A7B3] text-white' : 'bg-gray-100 text-gray-800 hover:bg-gray-200'}`}
                onClick={() => {
                  toast.success('Status set to Active');
                  if (authUser) authUser.driver_status = 'active';
                }}
              >
                Active
              </button>
              <button
                className={`px-4 py-2 rounded-lg font-semibold transition-colors border border-gray-200 ${authUser?.driver_status === 'available' ? 'bg-[#00A7B3] text-white' : 'bg-gray-100 text-gray-800 hover:bg-gray-200'}`}
                onClick={() => {
                  toast.success('Status set to Available');
                  if (authUser) authUser.driver_status = 'available';
                }}
              >
                Available
              </button>
              <button
                className={`px-4 py-2 rounded-lg font-semibold transition-colors border border-gray-200 ${authUser?.driver_status === 'inactive' ? 'bg-[#00A7B3] text-white' : 'bg-gray-100 text-gray-800 hover:bg-gray-200'}`}
                onClick={() => {
                  toast.success('Status set to Inactive');
                  if (authUser) authUser.driver_status = 'inactive';
                }}
              >
                Inactive
              </button>
